    ...
    <div class="card-body">
        <table class="table table-bordered ajax-crud-datatable">
            ...
            <tr>
                ...
                <td>{{$a->shape}}</td>
                <td><img src="{{ asset($a->image)}}" style="width: 40px; height: 40px; border-radius:0%;"/> </td>
                <td>{{$a->flavors->type}}</td>
                <td>{{$a->sizes->title}}</td>
                <!-- <td></td> -->
                <td>
                    <a class="btn btn-success" href="{{url('/vendor/custom-order-details/'.$a->id)}}">Details</a>
                </td>
            </tr>
            @endforeach
            </tbody>
