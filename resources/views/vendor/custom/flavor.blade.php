            ...
            <div class="modal-body">
                ...
                {{ csrf_field() }}
                    ...
                    <label for="name" class="col-sm-2 control-label">Type</label>
                        <div class="col-sm-12">
                            <input type="text" id="add-type" class="form-control" name="type" placeholder="Enter Type" maxlength="50" required="">
                        </div>
                    </div> 
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="javascript:void(0)" id="EditForm" class="form-horizontal" method="POST" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    <input type="hidden" name="id" id="flavor-id"> 
                    <div class="form-group">
                    <label for="name" class="col-sm-2 control-label">Type</label>
                        <div class="col-sm-12">
...
function add(){
    ...
    $('#AddModal').html("Add Flavor");
    $('#add-modal').modal('show');
}

$('#btn-save').on('click',function(){

var type = $('#add-modal').find('#add-type').val();
...
$.ajax({
    method:"POST",
    url: "{{ url('vendor/custom-cake/flavor/store') }}",
    dataType: 'json',
    processData: false,
    contentType: false,
    data: data,
    success: function(res){
        window.location = '{{ url("vendor/custom-cake/flavor")}}';
    },
});
...
function editFunc(id,obj){
    flavorId = id;
    $('#EditModal').html("Edit Flavor");
    $('#edit-modal').modal('show');
    $.ajax({
        type:"GET",
        url: "{{ url('vendor/custom-cake/flavor/edit') }}/" + id, 
        dataType: 'json',
        success: function(res){
            $('#edit-type').val(res.type);
            $('#edit-price').val(res.price);
            $('#flavor-id').val(res.id);
        }
    })
}
...
var id = $('#edit-modal').find('#flavor-id').val();
var type = $('#edit-modal').find('#edit-type').val();
...
$.ajax({
    method:"POST",
    url: "{{ url('vendor/custom-cake/flavor/update') }}/" + flavorId, 
    dataType: 'json',
    processData: false,
    contentType: false,
    data: data,
...
function deleteFunc(id, obj){
    ...
    $.ajax({
    type:"GET",
    url: "{{ url('vendor/custom-cake/flavor/delete') }}/" + id,
    dataType: 'json',
    success: function(res){
        window.location = '{{ url("vendor/custom-cake/flavor")}}';
    }
